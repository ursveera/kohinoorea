...
public static class PlanExpiredHtmlRenderer
{
    public static string Render(ProductDto product, ActivePlanDto plan)
    {
        ...
        var validTo = plan.ValidToUtc?.ToLocalTime().ToString("dd MMM yyyy") ?? "recently";
        ...
        return Wrap(
            title: "Plan Expired",
            eyebrow: "KOHINOOR EA • PLAN EXPIRED",
            heading: "Your plan has expired",
            bodyHtml: $@"
              <p style=""margin:0;color:#9ca3af;line-height:1.7"">
                Hi {Escape(name)}, your <strong style=""color:#f3f4f6"">{Escape(product.Name)}</strong> plan expired on
                <strong style=""color:#f3f4f6"">{Escape(validTo)}</strong>.
              </p>
              <div style=""margin-top:14px;padding:14px;border-radius:16px;background:rgba(248,113,113,.08);border:1px solid rgba(248,113,113,.22);color:#fecaca;line-height:1.7"">
                To regain access, renew your plan. Reply to this email or open a support ticket from your dashboard — we’ll assist immediately.
              </div>
