(function () {
  ...
  function goTo(page, planKey) {
    window.location.href = page + "?plan=" + encodeURIComponent(planKey);
  }
  ...
  function initPricingFlow() {
    ...
    document.querySelectorAll("[data-flow-plan]").forEach(function (element) {
      element.addEventListener("click", function (event) {
        ...
        if (isLoggedIn()) {
          goTo("cart.html", planKey);
          return;
        }
        goTo("registration.html", planKey);
      });
    });
  }
  ...
  function initRegistrationFlow() {
    ...
    if (form) {
      form.addEventListener("submit", function (event) {
        event.preventDefault();
        goTo("login.html", selectedPlan.key);
      });
    }
  }
  ...
  function initLoginFlow() {
    ...
    setLink("[data-login-register-link]", "registration.html?plan=" + encodeURIComponent(selectedPlan.key));
    ...
    if (form) {
      form.addEventListener("submit", function (event) {
        event.preventDefault();
        setLoggedIn(true);
        goTo("cart.html", selectedPlan.key);
      });
    }
  }
  ...
  function initCartFlow() {
    ...
    setLink("[data-cart-buy-link]", "buy.html?plan=" + encodeURIComponent(selectedPlan.key));
  }
  ...
  function initBuyFlow() {
    function syncBuyPlan(planKey) {
      ...
      setLink("[data-buy-cart-link]", "cart.html?plan=" + encodeURIComponent(selectedPlan.key));
  ...
  if (document.body.hasAttribute("data-page-pricing")) {
    initPricingFlow();
  }

  if (document.body.hasAttribute("data-page-registration")) {
    initRegistrationFlow();
  }

  if (document.body.hasAttribute("data-page-login")) {
    initLoginFlow();
  }

  if (document.body.hasAttribute("data-page-cart")) {
    initCartFlow();
  }

  if (document.body.hasAttribute("data-page-buy")) {
    initBuyFlow();
  }
}());